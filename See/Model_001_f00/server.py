...
def run_server(prediction_fn, gt_fn):
  ...
  num_correct_empty = sum(1 for s in scores
      if s[-1] == 'empty' and s[1] == 'empty')
  num_correct_non_empty = sum(1 for s in scores
      if s[-1] == 'non-empty' and s[1] == 'non-empty')
  ...
  print("[GT: %5d | P: %5d | CORRECT: %5d] %012s %.4f | %.4f" % (num_empty,
      num_empty_pred, num_correct_empty,
      'Empty: ', empty_score / num_empty, empty_score / len(scores)))
  print("[GT: %5d | P: %5d | CORRECT: %5d] %012s %.4f | %.4f" % (num_non_empty,
      num_non_empty_pred, num_correct_non_empty,
      'Non-Empty: ', non_empty_score / num_non_empty,
